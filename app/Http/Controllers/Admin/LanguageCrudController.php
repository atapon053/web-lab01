...
class LanguageCrudController extends CrudController
{
    ...
    private function addDefaultField($id = null)
    {
        $attributes = [];
        $localeDefault = $this->crud
            ->getModel()
            ->select('id', 'name')
            ->first();
        $hintDefaultLocale = '';

        if ($localeDefault && $localeDefault->id != $id) {
            $hintDefaultLocale = '<br/> มีการกำหนดภาษาเริ่มต้นภาษา ' . $localeDefault->name . ' แล้ว';
            $attributes = [
                'disabled' => 'disabled'
            ];
        }
